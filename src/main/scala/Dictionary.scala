object Dictionary {
    ...
    FileIO.readDictionaryFile(filePath).map { lines =>
      lines.map { name =>
        entityType match {
          ...
          case "Tech  nology"          => new Technology(name)
          case "ProgrammingLanguage" => new ProgrammingLanguage(name)
          case _                     => new Person(name) // fallback
        }
      }
    }.orElse {
      println(s"Warning: Could not load $filePath/$entityType.txt")
      None
    }
  }
   ...
   */
  def loadAll(entitiesDir: String): Option[List[NamedEntity]] = {
